...
export default class Trails extends THREE.Mesh {
  constructor(renderer, num, length, options) {
    const geometry = new THREE.OctahedronGeometry(1, 20);
    ...
    this.length = 1;
    this.num = geometry.attributes.position.array.length/3;
    this.time = 0;
    // this.material.needsUpdate = true
    this.matrixAutoUpdate = false;
    this.updateMatrix();
    this.computeRenderer = new GPUComputationRenderer(this.length, num, renderer);
    this.computeValues = {
      position: {
        texture: null,
        uniforms: null
